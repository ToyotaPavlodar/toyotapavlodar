...
async function assertDashboard(context: {
...
async function loadDashboard(context: DashContext, data: { from: string; to: string }) {
  ...
  const [leadStats, { data: spend }, { data: brands }, assigneeRes, avgRate, { data: latestFx }] =
    await Promise.all([
      ...
      context.supabase
        ...
        .gte("date", fromDate)
        .lt("date", toExclusive.toISOString().slice(0, 10)),
      context.supabase
        .from("brands")
  ...
  const byBrand = brandSlices
    ...
    .map((slice) => {
      const bSpendUsd = spendRows
        .filter((s) => s.brand_id === slice.id)
        .reduce((a, r) => a + Number(r.spend_usd), 0);
      const bSpendKzt = bSpendUsd * avgRate;
      const bFunnel = computeBrandCrmFunnel(leadRows, slice.id, slice.table_leads);
      const bCosts = computeCostMetrics(
        bSpendKzt,
        slice.total_leads,
        bFunnel.qualified,
        bFunnel.sent_to_1c,
  ...
  const trendMessagingTotals = await fetchMessagingTotalsByMonth(context.supabase, trendMonthKeys, {
  ...
  return {
    ...
    month: isFullMonthPeriod(data.from, data.to) ? data.from.slice(0, 7) : null,
    scope: {
      brand_id: brandScope,
    ...
    avg_rate: avgRate,
    latest_rate: latestFx?.[0]
      ? { date: latestFx[0].date, usd_kzt: Number(latestFx[0].usd_kzt) }
      : null,
    totals: {
...
export const exportMonthlyReportCsv = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const filename =
      d.period.is_full_month && d.month
        ? `otchet-${d.month}.csv`
