    ...
    <LegalPageLayout title="Инструкция по удалению данных">
      ...
      <LegalSection title="2. Что будет удалено">
        ...
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>
            связанные технические записи (ID формы, кампании) — если они позволяют идентифицировать
            вас.
          </li>
        </ul>
        <p>
          Данные, которые мы обязаны хранить по закону (бухгалтерские документы, оформленные сделки
          в 1С), могут быть сохранены в обезличенном виде или в объёме, требуемом законодательством.
        </p>
      </LegalSection>
      ...
      <LegalSection title="5. Meta (Facebook / Instagram)">
        <p>
          Если вы хотите отозвать доступ приложения Meta к вашим данным на стороне Facebook, это
          также можно сделать в настройках Facebook: «Настройки и конфиденциальность» → «Центр
          аккаунтов» → «Ваши данные и разрешения». Удаление данных в нашей CRM выполняется отдельно
          по запросу, описанному выше.
        </p>
      </LegalSection>
    </LegalPageLayout>
  );
